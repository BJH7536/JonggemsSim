...
(function () {
  ...
  // 발판 축소 (2026-08-09, 플레이 피드백): 발판 크기를 절반으로. 폭만 반으로 줄이면
  // 지그재그 사다리의 수평 틈이 사거리(128)를 넘어간다 — 실측 최대 188. 그래서 중심을
  // 화면 중앙(480) 쪽으로 0.7 모아 틈을 원래 수준(110→108)으로 되돌린다. 윗면 y는
  // 그대로라 수직 간격은 불변. 위 표의 좌표는 축소 전 원본 배치다 (selftest가 결과를 검증).
  ROUTE.forEach(function (r) {
    r.x = Math.round(480 + (r.x + r.w / 2 - 480) * 0.7 - r.w / 4);
    r.w = Math.round(r.w / 2); r.h = Math.round(r.h / 2);
  });
  var TERRAIN = [R(0, GROUND_Y, 960, 80)].concat(ROUTE, ROCKS, [
    R(395, SUMMIT_Y, 110, 14),   // 정상 발판도 같이 반으로 (클리어 판정 x범위와 함께 수정)
  ...
  //   640(4판): 두 제약을 동시에 만족하는 구간 (620,656)의 중앙값.
  //   620(현재): 발판 축소로 첫 발판 오른쪽 끝이 600→540으로 물러나 640에선 114px
  //             (기준 110 초과). 구간 하단인 620으로 내리면 96px — 머리 위도 그대로 트임.
  var START_X = 620;
  ...
  function start(stage) {
    ...
    return {
      ...
      step: function (dt) {
        ...
        if (!st.cleared && st.py < SUMMIT_Y + 4 && st.px > 395 && st.px < 505) {
          st.cleared = true;
      ...
      TERRAIN.forEach(function (s) {
        ...
        if (s.t === 'c') {
          ...
          if (s.w <= 300 && imgReady('ledge')) {
            ctx.drawImage(IMG['ledge'], s.x - 2, s.y - 2, s.w + 4, s.h + 5);
          } else {
